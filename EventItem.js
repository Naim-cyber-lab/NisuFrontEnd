...
const EventItem = (props) => {
    ...
    useEffect(() =>{
      if((event.PreferenceEvent) != null){
        var myCloneList = {...event.PreferenceEvent};
    ...
    const renderItemPreference = ({item}) => {
        ...
        return(
            <Pressable onPress = {() => handleNavigationHastag() }>
                <Text style={{color:"white"}}>{item.hastag}</Text>
            </Pressable>
        )
    }
...
// ******************************************** POUR LE FLATLIST (SWIPER) ******************************************************************
    ...
    const renderFlatlist = ({ item, index }) => {
        return(
            ...
            <ItemFlatlist ipAdress={ipAdress}
             //parentRefIndex={parentRefIndex}
              event={event}  titre={event.titre} audio={event.audio} navigation={navigation} numberFiles={getFilesEvent[0].length} containVideo={getFilesEvent[1]} filesEvent={filesEvent} lengthFilesEvent={lengthFilesEvent} item={item} index={index} swiperRef={swiperRef} /> 
        )        
    }
...
// *******************************************************************************************************************************
    return (
        <View style={{marginTop:15,borderRadius:3,padding:10,marginBottom:15}}>
            <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between'}}>
                ...
                <View>
                    <Text style={{fontSize:30,color:"white"}}>{event.titre}</Text>
                </View>
                <View style={{display:'flex',flexDirection:'column',color:"white"}}>
                    <Text style={{color:"white"}}>{event.city}</Text>
                    <Text>12 Mars 2021</Text>
                </View>
            ...
            <View style={{backgroundColor:'black'}}>
                <View style={{position:'absolute',backgroundColor:'transparent',opacity:1,marginLeft:"80%",width:10,zIndex:1,bottom:0,alignItems:"center",justifyContent:'space-around'}}>
                
                <TouchableOpacity style={{padding:10}}  onPress={onModalComment}>
                    <Fontisto name={'comment'} size={30} style={{color:'white'}}
                    
                        // onPress={() => props.navigation.navigate('CommentItem',{'event':event})}
                    />                    
                </TouchableOpacity>

                <TouchableOpacity style={{padding:10}}  onPress={onModalPaperPlane}>
                    <Fontisto name={'paper-plane'} size={30} style={{color:'white'}} />
                </TouchableOpacity>
                ...
                <>
                    ...
                    <AnimatedImage
                        ...
                        <Video 
                            ...
                            style={{flex:1,width:widthScreen,height:250}}
            ...
            <View style={{backgroundColor:'transparent',justifyContent:'space-between',}}>
                ...
                <TouchableOpacity onPress={() => handleDemandeParticipation()} style={{...styles.iconContainer,justifyContent:"center",alignItems:"center",height:40,backgroundColor:"rgb(105, 131, 209)",borderRadius:20,opacity:0.4,padding:3}} >
                    <Text style={{color:"white",justifyContent:"center",alignItems:"center"}}>{ event.accessOuvert ? "Participation libre" : "Participer"}</Text>
                </TouchableOpacity>
            </View>
